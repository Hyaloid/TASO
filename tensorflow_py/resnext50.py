...
def resnext_block(input, strides, out_channels, groups, name):
    ...
    for i in range(groups):
        t[i] = make_conv2d(input_tensor=t[i], filter_shape=(3, 3, t[i].shape[1].value, out_channels // groups),
                           strides=strides, padding="SAME", actimode="RELU", name=name + "_conv2_".format(i))
    output = tf.concat(t, axis=1, name=name + "_concat")
    t = make_conv2d(input_tensor=output, filter_shape=(1, 1, output.shape[1].value, 2 * out_channels),
                    strides=(1, 1, 1, 1), padding="SAME", actimode="NONE", name=name + "_conv3")
    if (strides[2] > 1) or (input.shape[1].value != out_channels * 2):
        input = make_conv2d(input_tensor=input, filter_shape=(1, 1, input.shape[1].value, out_channels * 2),
