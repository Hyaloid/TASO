...
def squeeze(out_channels, input):
    return make_conv2d(input_tensor=input, filter_shape=(1, 1, input.shape[1].value, out_channels),
                       strides=(1, 1, 1, 1), padding="SAME", actimode="RELU", name="squeeze")
...
def fit(current, input):
    if (input.shape[2].value == current.shape[2].value):
        ...
        return make_conv2d(input_tensor=input, filter_shape=(3, 3, input.shape[1].value, current.shape[1].value),
...
def normal_cell(prev, cur, out_channels):
    ...
    prev = fit(cur, prev)
    ts = list()
    ts.append(make_seperable_conv2d(input_tensor=cur, out_channels=out_channels, kernels=(3, 3), strides=(1, 1, 1, 1),
                                    padding="SAME"))
    ts.append(cur)
    ts.append(make_seperable_conv2d(input_tensor=prev, out_channels=out_channels, kernels=(3, 3), strides=(1, 1, 1, 1),
                                    padding="SAME"))
    ts.append(make_seperable_conv2d(input_tensor=cur, out_channels=out_channels, kernels=(3, 3), strides=(1, 1, 1, 1),
                                    padding="SAME"))
...
def reduction_cell(prev, cur, out_channels):
    ...
    ts = list()
    outputs = list()
    ts.append(make_seperable_conv2d(input_tensor=prev, out_channels=out_channels, kernels=(7, 7), strides=(1, 1, 2, 2),
                                    padding="SAME"))
    ts.append(make_seperable_conv2d(input_tensor=cur, out_channels=out_channels, kernels=(5, 5), strides=(1, 1, 2, 2),
                                    padding="SAME"))
    ...
    ts.append(make_seperable_conv2d(input_tensor=prev, out_channels=out_channels, kernels=(5, 5), strides=(1, 1, 2, 2),
                                    padding="SAME"))
    outputs.append(tf.add(ts[4], ts[5]))
    ts.append(make_maxpool2d(input_tensor=cur, kernels=(1, 1, 3, 3), strides=(1, 1, 2, 2), padding="SAME"))
    ts.append(
        make_seperable_conv2d(input_tensor=outputs[0], out_channels=out_channels, kernels=(3, 3), strides=(1, 1, 1, 1),
                              padding="SAME"))
...
input0 = tf.compat.v1.placeholder(tf.float32, shape=(1, 128, 56, 56))
input = input0
out_channels = 128
for i in range(3):
